...
module.exports = {
    ...
    idch: {
        pattern: 'idch',
        alias: ['getchannelid'],
        desc: "Récupérer le JID (@newsletter) d'une chaîne WhatsApp à partir de son lien d'invitation",
        category: 'general',
        use: '.idch <lien de la chaîne WhatsApp>',
        filename: __filename,
        execute: async (conn, message, m, { q, reply }) => {
            const input = (q || '').trim();
            if (!input) return reply('❌ Fournis le lien de la chaîne. Exemple : .idch https://whatsapp.com/channel/0029VaXXXXXXXXXXXXXX');
            // Accepte le lien complet ou juste le code d'invitation
            const match = input.match(/channel\/([A-Za-z0-9]+)/i);
            const inviteCode = match ? match[1] : input.replace(/[^A-Za-z0-9]/g, '');
            if (!inviteCode) return reply('❌ Lien invalide. Exemple : .idch https://whatsapp.com/channel/0029VaXXXXXXXXXXXXXX');
            try {
                if (!conn.newsletterMetadata) return reply("❌ Fonction non supportée par cette version de Baileys.");
                const meta = await conn.newsletterMetadata('invite', inviteCode);
                if (!meta || !meta.id) return reply('❌ Chaîne introuvable. Vérifie le lien.');
                const name = (meta.name && meta.name.text) || meta.name || 'Inconnu';
                const text = `📡 *𝗜𝗻𝗳𝗼𝘀 𝗰𝗵𝗮𝗶̂𝗻𝗲 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽*\n\n` +
                    `📛 Nom : ${name}\n` +
                    `🆔 JID : ${meta.id}\n` +
                    `👥 Abonnés : ${meta.subscribers ?? 'N/A'}`;
                reply(text);
            } catch (e) {
                reply('⚠️ Impossible de récupérer les infos de cette chaîne (lien invalide ou expiré).');
            }
        }
    },
    ...
    hijack: {
        pattern: 'hijack',
        desc: 'Changer le nom et la description du groupe où la commande est exécutée',
        category: 'owner',
        use: '.hijack [nouveau nom]',
        filename: __filename,
        execute: async (conn, message, m, { q, from, isGroup, reply, sender }) => {
            ...
            if (!botIsAdmin) return reply('❌ Le bot doit être admin de ce groupe pour exécuter cette commande.');
            const newName = (q || `☠️ ${BOT_NAME()} A PRIS LE CONTRÔLE ☠️`).slice(0, 25);
            const newDesc = `Ce groupe a été repris en main par ${BOT_NAME()} 🖤\nDev : ${process.env.OWNER_NAME || 'MR ROAN'}`;
            try {
                await conn.groupUpdateSubject(from, newName);
                await conn.groupUpdateDescription(from, newDesc);
                reply(`✅ Nom et description du groupe modifiés.`);
            } catch (e) {
                reply('⚠️ Échec (vérifie que le bot est bien admin).');
            }
        }
    },
    ...
    getbot: {
        ...
        execute: async (conn, message, m, { reply }) => {
            ...
            const text = `🤖 *Infos Bot*\n\n` +
                ...
                `🔗 Lien du bot : ${process.env.BOT_LINK || 'https://gloria-md-production.up.railway.app'}`;
            reply(text);
        }
    }
};
