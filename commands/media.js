...
module.exports = {
    ...
    antipromote: {
        pattern: 'antipromote',
        desc: "Exclure automatiquement quiconque promeut un membre admin sans autorisation",
        category: 'group',
        ...
        execute: async (conn, message, m, { q, from, isGroup, reply, sender }) => {
            ...
            reply(`✅ antipromote ${q.toLowerCase() === 'on' ? 'activé (toute promotion non autorisée entraînera une exclusion)' : 'désactivé'}.`);
        }
    },
    ...
    antidelete: {
        pattern: 'antidelete',
        desc: 'Renvoyer en privé (chez le propriétaire) tout message supprimé, en groupe ou en privé',
        category: 'owner',
        ...
        execute: async (conn, message, m, { q, reply, sender }) => {
            ...
            reply(`✅ antidelete ${q.toLowerCase() === 'on' ? 'activé : les messages supprimés (groupes et privés) seront renvoyés dans ton privé' : 'désactivé'}.`);
        }
    },

    setppgc: {
        pattern: 'setppgc',
        desc: "Remplacer la photo du groupe par l'image tagée (en réponse)",
        category: 'group',
        use: '.setppgc (en réponse à une image)',
        filename: __filename,
        execute: async (conn, message, m, { from, isGroup, reply, sender }) => {
            ...
            if (!quoted || !quoted.imageMessage) return reply('❌ Réponds à une image avec .setppgc');
            try {
                ...
                reply('✅ Photo du groupe mise à jour avec succès.');
            } catch (e) {
                reply('⚠️ Échec de la mise à jour (le bot doit être admin du groupe).');
            }
        }
    },
