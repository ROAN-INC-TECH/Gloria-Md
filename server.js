...
const AUTO_STATUS_MSG = process.env.AUTO_STATUS_MSG || "YOUR STATUS HAS BEEN SEEN BY GLORIA-MD 🖤";
const DEV = process.env.DEV || '𝗠𝗥 𝗥𝗢𝗔𝗡';
    ...
    try {
        ...
        if (isGroup0 && store.isAntilink(from0) && body) {
            ...
            if (linkRegex.test(body)) {
                ...
                try {
                    const meta = await conn.groupMetadata(from0);
                    const p = meta.participants.find(x => x.id === senderJid);
                    senderIsAdmin = p?.admin === 'admin' || p?.admin === 'superadmin';
                } catch (e) {}
        ...
        if (from.endsWith('@newsletter')) {
            ...
            switch (commandName) {
                case 'ping':
                    ...
                    const details = `⚡ *${BOT_NAME} 𝗦𝗽𝗲𝗲𝗱 𝗖𝗵𝗲𝗰𝗸* ⚡
                    
⏱️ 𝗧𝗶𝗺𝗲 𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲: *${responseTime.toFixed(2)}s* ⚡
👤 𝗢𝘄𝗻𝗲𝗿: *${OWNER_NAME}*`;
        ...
        switch (commandName) {
            case 'prefix':
                ...
                const currentPrefix = store.getPrefix(sessionId, userPrefixes.get(sessionId) || PREFIX);
                await conn.sendMessage(from, { 
...
👋 𝗛𝗜 *${name}* 🤩  
🎉 𝗣𝗮𝗶𝗿𝗶𝗻𝗴 𝗖𝗼𝗺𝗽𝗹𝗲𝘁𝗲 – 𝗕𝗼𝘁 𝗜𝘀 𝗔𝘃𝗮𝗶𝗹𝗮𝗯𝗹𝗲
...
${channelStatus}
    ...
    conn.ev.on('presence.update', ({ id, presences }) => {
        try {
            ...
            for (const [jid, data] of Object.entries(presences)) {
                presence.update(jid, data?.lastKnownPresence);
            }
        } catch (e) {}
    });
    ...
    setInterval(async () => {
        try {
            ...
            for (const [groupId, sched] of Object.entries(schedules)) {
                if (sched.openTime === hhmm && sched.lastOpenRun !== todayKey) {
                    try { await conn.groupSettingUpdate(groupId, 'not_announcement'); } catch (e) {}
                    store.setSchedule(groupId, { lastOpenRun: todayKey });
                }
                if (sched.closeTime === hhmm && sched.lastCloseRun !== todayKey) {
                    try { await conn.groupSettingUpdate(groupId, 'announcement'); } catch (e) {}
                    store.setSchedule(groupId, { lastCloseRun: todayKey });
                }
            }
        } catch (e) {}
    }, 60 * 1000);
    ...
    conn.ev.on("messages.upsert", async (m) => {
        try {
            ...
            if (message.message && from !== 'status@broadcast') {
                ...
                if (store.getToggle('autotyping')) {
                    conn.sendPresenceUpdate('composing', from).catch(() => {});
                }
                if (store.getToggle('autorecording')) {
                    conn.sendPresenceUpdate('recording', from).catch(() => {});
                }
                if (store.getToggle('autoreact') && !message.key.fromMe) {
                    const emojis = ['❤️', '🔥', '👍', '😊', '✨', '🎉'];
                    const emoji = emojis[Math.floor(Math.random() * emojis.length)];
                    conn.sendMessage(from, { react: { text: emoji, key: message.key } }).catch(() => {});
                }
            }
...
function gracefulShutdown() {
  ...
  isShuttingDown = true;
  console.log("\n🛑 Shutting down GLORIA-MD server...");
